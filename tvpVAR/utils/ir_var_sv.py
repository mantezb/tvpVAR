...
def ir_sim(i:int, s:int, n: int, k: int, p: int, b0t: np.ndarray, ltidx: np.ndarray, cidx: np.ndarray, bidx:np.ndarray, bigj: np.ndarray, sigma: np.ndarray, beta: np.ndarray, t_start:np.ndarray, pol_eq : int) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """
    ...
    :param k: no of parameters across all var equations
    :param p: no of lags
    ...
    for j in range(beta.shape[1]):
        ...
        if j == t_start[3]:
            ...
            for ij in range(s + 1):
                jj = jj + n
                impf_m[:, ij] = imp_resp[:, jj]  # restrict to the n'th equation, the interest rate
            impf_m4 = impf_m  # store draws of responses
        ...
        if j == t_start[4]:
            ...
            for ij in range(s + 1):
                jj = jj + n
                impf_m[:, ij] = imp_resp[:, jj]  # restrict to the n'th equation, the interest rate
            impf_m5 = impf_m  # store draws of responses
        ...
        if j == t_start[5]:
            ...
            for ij in range(s + 1):
                    jj = jj + n
                    impf_m[:, ij] = imp_resp[:, jj]  # restrict to the n'th equation, the interest rate
            impf_m6 = impf_m  # store draws of responses

    return impf_m1, impf_m2, impf_m3, impf_m4, impf_m5, impf_m6


def ir_var_sv(beta: np.ndarray, sigma: np.ndarray, cidx: np.ndarray, t_start: np.ndarray, s: int, p: int, pol_eq: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """
    ...
    # Allocating space for storage of the results
    ir1 = np.zeros((n, 1 + s, nsims))
    ir2 = np.zeros((n, 1 + s, nsims))
    ir3 = np.zeros((n, 1 + s, nsims))
    ir4 = np.zeros((n, 1 + s, nsims))
    ir5 = np.zeros((n, 1 + s, nsims))
    ir6 = np.zeros((n, 1 + s, nsims))
    ...
    #res = Parallel(n_jobs=cores)(delayed(ir_sim)(i, s, n, k, p, b0t, ltidx, cidx, bidx, bigj, sigma, beta, t_start, pol_eq) for i in tqdm(range(nsims)))
    res = []
    for i in tqdm(range(nsims)):
        res.append(ir_sim(i, s, n, k, p, b0t, ltidx, cidx, bidx, bigj, sigma, beta, t_start, pol_eq))
    ...
    return ir1, ir2, ir3, ir4, ir5, ir6
